...
1. **两台机器都打开模拟器面板**，至少房主一侧加载好游戏（ROM 可由房主自动推送给对手，对手无需提前准备同一 ROM）。
2. 点击游戏控制栏的 **📡 联机** 按钮打开联机弹窗，也可以通过命令面板执行 `红白机模拟器：创建房间` / `红白机模拟器：加入房间` 打开。
3. **房主**：在「创建房间」标签页输入端口（留空 = 随机端口）后点 **创建房间**。创建成功后会列出本机局域网 IP 与端口，每行附 **复制** 按钮，把地址发给对手即可。创建者固定为 **1P**。
4. **对手**：切到「加入房间」标签页，输入房主的 `IP:端口` 后点 **连接**。加入者固定为 **2P**。
5. 连接建立后弹窗自动切换为状态面板。房主点击 **同步存档** 把当前游戏状态（及 ROM）发给对手，对手加载完成后房主点 **开始游戏** 即可对战。
6. 结束时在弹窗或联机状态条点 **断开**，或执行 `红白机模拟器：断开连接`。

> 联机时双方都用自己的 **P1 按键配置**（W/A/S/D 等）操作各自的角色，无需切换到 P2 按键。
